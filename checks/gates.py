...
@gate('verify.py all-pass and Part A matches the receipts')
def g_verify():
    # The receipt comparison is folded in here rather than made a 23rd gate:
    # g_countline enforces the gate count, which is printed on the cover and in
    # three README strings, so adding one ripples.  receipts.py asserts every
    # Part A row is the live receipt verbatim -- the unguarded duplication that
    # let rows 27, 54 and 55 go stale in a Part A whose opening line claims its
    # rows are generated by running the suite, not transcribed.
    p = subprocess.run([sys.executable, os.path.join(HERE, 'verify.py')],
                       capture_output=True, text=True,
                     encoding='utf-8', errors='replace', timeout=1800)
    m = re.search(r'---- (\d+)/(\d+) checks pass, (\d+) audits pass', p.stdout)
    rec = subprocess.run([sys.executable, os.path.join(HERE, 'receipts.py')],
                         capture_output=True, text=True,
                         encoding='utf-8', errors='replace', timeout=1800)
    ok = bool(p.returncode == 0 and m and m.group(1) == m.group(2)
              and rec.returncode == 0)
    detail = (m.group(0) if m else 'no summary') + (
        '; Part A in sync' if rec.returncode == 0
        else '; PART A OUT OF SYNC -- run checks/receipts.py --write')
    return ok, detail
