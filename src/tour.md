...
<!--TEX: % KW, the anchor -- drawn, not spelled in ASCII
\par\medskip\noindent
...
4. The circle: in the plane sum-zero the three walls are pairwise at 60 degrees exactly, $\cos^2 = 1/4$ as an exact rational. A search over 4000 lattice directions realises all six arcs, none of them empty. [check 4]
...
292 and 289 of them, each five distinct integer utilities drawn without
replacement from $-8 \dots 8$ with weights uniform on $1 \dots 12$ and
normalised. Spanning families, not universal quantifiers. The dominance
...
> **KEY CLAIM (The triangle's numbers come from sum-to-zero PLUS equal norms, not from the word "triangle").**
> Project the three standard basis vectors of $R^3$ orthogonally to $(1,1,1)$. The results sum to zero, each has squared norm $2/3$, and every pairwise inner product is exactly $-1/3$, so every normalised cosine is exactly $-1/2$ -- all in rational arithmetic, with no square roots anywhere [check 45]. The one-line reason: $|\sum w_i|^2 = 3 + 2\sum_{i<j}\cos_{ij} = 0$. And the word "triangle" alone does **not** fix this: an exhaustive search finds 80 equal-cosine planar triples whose common $\cos^2$ is not $1/4$; 8 of them do sum to zero, and what rules out all 80 is that none has **equal norms** -- the condition the displayed identity assumes when it writes the norm sum as $3$ [check 48]. [Theorem] for `TRI`.
...
46. The bound, and why the construction is a projection. Over all triples of 24 rational planar directions the largest pairwise $\cos^2$ is never below $1/4$ and **never equal to it** -- the bound itself is Welch [primary-verified: IEEE Transactions on Information Theory IT-20(3), pp. 397-399, 1974] -- the best achievable is $9/34$. Sixty degrees needs $\tan = \sqrt{3}$, so no rational planar triple attains the optimum. The projected frame attains it exactly. [check 46]
...
| 20 | 4 | lineality space is exactly the 11 constant vectors -- one per value of the 11-value pool -5..5 -- in this check's grid of all 11^3 tuples with repeats allowed; the distinct-valued grid of checks 19 and 21 contains no constant vector at all, which is why this check needs its own grid; the constant reward lies in all 3 cones simultaneously and makes every policy optimal |
...
| 32 | 6 | a train/test distinguisher multiplies the consistent set by exactly 343 -- the size of the hypothesis box (-3..3)^3, 7 integer weights per feature -- and drives the generalising fraction to the prior 153/343 (153 of the 343 box points prefer the true feature at test), i.e. training carries zero information about test behaviour; without the flag it carries 55/153 (55 of the 153 strictly train-optimal points), which is WORSE than the prior -- correlated training actively misinforms |
| 33 | 6 | over the 343-point integer box (-3..3)^3 the training-indistinguishable directions form a subspace: 37 grid points on one comparison -- the plane w1+w2+w3=0, dimension 2 -- and 7 on two -- the line w1=0, w2=-w3, dimension 1. The two counts are lattice-point counts of that plane and that line and move with the box; the dimension drop from 2 to 1 does not. Same cone-and-lineality structure as Chapter 4, with the training set doing the observing |
| 34 | 6 | consistent objectives fall 153 -> 55 -> 37 -> 19 over the 343-point weight box (-3..3)^3, as the comparisons 111>000, 100>011, 110>001, 101>010 are imposed cumulatively in that order, each as a strict preference; every survivor of full coverage has positive weight on the true feature |
...
| 41 | 8 | over all 6561 depth-3 trees -- every assignment of leaf values from {0,1,2} to the 8 leaves, 3^8 -- and 3 overclaims each, the true max plus 1 / 2 / 5: honesty wins every time under a correct judge and loses every time under an inverted one |
...
| 48 | 9 | exhaustive over all 17296 unordered triples of the 48 non-zero integer directions in (-3..3)^2: 80 have a common pairwise cos^2 other than 1/4, every one of them 1; 8 of the 80 DO sum to zero, so sum-to-zero alone does not force 1/4 -- 0 have equal norms, and equal norms is the condition that does |
...
| 57 | c2c | every one of 391 distinct behaviours in a 6-planner by 216-reward class admits at least 2 decompositions, and up to 36; no behaviour identifies its pair. The rationality confound is only partly reachable: a beta-partner needs w^beta, which leaves any finite pool. The planners are Luce with exponent beta in {+/-1, +/-2, +/-3}, negative meaning the minimiser u = w^-beta; the 216 reward weights are all 6^3 tuples over the pool {1,2,3,4,6,12}, chosen CLOSED under x -> 12/x so that a minimiser's reciprocal partner rescales back into the grid -- on the un-closed pool {1,2,3,4,5,6} the same sweep leaves 432 singleton fibres and the 'at least 2' fails; rewards are r = log w. The maximum 36 is |pool| x |planners|, the one uniform behaviour shared by the constant tuples, not a property of this pool |
...
functions can be unhackable only if one of them is constant or the two are
positive affine transforms of one another -- check 18 finds 60 pairs where both
are non-constant, 36 of them with the two rewards distinct. Non-trivial
...
bibliographic assertions, fourteen folklore verdicts, forty-one printed life spans
