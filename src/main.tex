...
\vfill
%% WARNING: the seven numbers printed below duplicate the COUNTLINE comment above,
%% but gates.py's g_countline() parses ONLY the comment -- it never reads these
%% printed values.  Editing COUNTLINE without editing these ships a wrong number on
%% the cover.  That happened once already: inserting "How this was written" took
%% starred from 9 to 10 and this line still said 9.  Keep them in sync by hand until
%% g_countline() is extended to compare the printed line too.
